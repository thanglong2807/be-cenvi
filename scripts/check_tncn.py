#!/usr/bin/env python3
"""
Script kiểm tra folder THU-NHAP-CA-NHAN trên Drive.
Nếu folder rỗng hoặc không tồn tại -> đánh warning.
Nếu có file/folder -> giữ nguyên trạng thái.
"""
...
# ---------------------------------------------------------------------------
# Quét folder THU-NHAP-CA-NHAN theo quý
# ---------------------------------------------------------------------------
def check_tncn_files(company_root_id: str, year: int, period: str) -> dict:
    """
    Kiểm tra folder THU-NHAP-CA-NHAN trong TAI-LIEU-THUE/{year} theo quý.
    - Không tìm thấy folder TAI-LIEU-THUE -> warning
    - Không tìm thấy folder năm -> warning
    - Không tìm thấy folder THU-NHAP-CA-NHAN -> warning
    - Không tìm thấy folder quý -> warning
    - Folder quý rỗng -> warning
    - Có ít nhất 1 file trong quý -> pass
    """
    try:
        ...
        if not f_tncn:
            return {'status': 'warning', 'message': 'Khong tim thay folder THU-NHAP-CA-NHAN', 'files_count': 0}

        # 4. Tìm folder quý
        f_quarter = _find_quarter_folder(service, f_tncn['id'], q_num)
        if not f_quarter:
            return {'status': 'warning', 'message': f'Khong tim thay folder {period} trong THU-NHAP-CA-NHAN', 'files_count': 0}

        # 5. Kiểm tra có file trong quý không
        files = _call_with_retry(get_all_files_recursive, service, f_quarter['id'])
        if not files:
            return {'status': 'warning', 'message': f'Folder {period} trong THU-NHAP-CA-NHAN rong', 'files_count': 0}

        return {'status': 'pass', 'message': f'Tim thay {len(files)} file trong THU-NHAP-CA-NHAN/{period}', 'files_count': len(files)}
...
def _mark_as_missing(db: Session, folder_id: int, year: int, period: str, session_id: int | None, message: str):
    note = f"[He thong] Tu dong quet TNCN {period}: {message}"
    try:
        if session_id:
...
# Hàm chính
# ---------------------------------------------------------------------------
def check_missing_tncn(year: int, period: str, force: bool = False):
    settings.DOCKER = False

    print(f"Bat dau chay Script: Kiem tra TNCN cho Nam {year} - {period}")
    ...
    for idx, folder in enumerate(all_folders, 1):
        ...
        try:
            result = check_tncn_files(root_id, year, period)
    ...
    print(f"  Thieu TNCN (warning)       : {count_missing}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Kiem tra file TNCN tren Drive theo quy.")
    parser.add_argument("--year",   type=int, default=datetime.now().year, help="Nam can kiem tra (mac dinh: nam hien tai)")
    parser.add_argument("--period", type=str, default=None,                help="Quy can kiem tra: Q1, Q2, Q3, Q4. Bo trong de check ca 4 quy.")
    ...
    print(f"\n{'='*50}")
    print(f"BAT DAU KIEM TRA TNCN NAM {args.year}" + (f" - {args.period.upper()}" if args.period else " (CA 4 QUY)"))
    print(f"{'='*50}\n")
    ...
    for q in quarters:
        ...
        print(f"{'#'*50}")
        check_missing_tncn(args.year, q, force=args.force)
