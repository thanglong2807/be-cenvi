...
class DocumentService:
    ...
    def _ensure_dict_metadata(self, obj):
        """Ensure Metadata field is always a dict, not a string"""
        if hasattr(obj, 'Metadata'):
            obj.Metadata = self._normalize_metadata_value(obj.Metadata)
        return obj
    ...
    def get_full_document_tree(self) -> List[DanhMuc]:
        ...
        for cat in categories:
            cat.Metadata = self._normalize_metadata_value(getattr(cat, 'Metadata', None))
            for proj in getattr(cat, 'muc_con', []) or []:
                if hasattr(proj, 'Metadata'):
                    proj.Metadata = self._normalize_metadata_value(getattr(proj, 'Metadata', None))
                for doc in getattr(proj, 'tai_lieu', []) or []:
                    doc.Metadata = self._normalize_metadata_value(getattr(doc, 'Metadata', None))
                    cv = getattr(doc, 'current_version', None)
                    if cv is not None and hasattr(cv, 'Metadata'):
                        cv.Metadata = self._normalize_metadata_value(getattr(cv, 'Metadata', None))
    ...
    def get_category_detail(self, dm_id: int) -> DanhMuc:
        ...
        category.Metadata = self._normalize_metadata_value(getattr(category, 'Metadata', None))
        for proj in getattr(category, 'muc_con', []) or []:
            if hasattr(proj, 'Metadata'):
                proj.Metadata = self._normalize_metadata_value(getattr(proj, 'Metadata', None))
            for doc in getattr(proj, 'tai_lieu', []) or []:
                doc.Metadata = self._normalize_metadata_value(getattr(doc, 'Metadata', None))
